...
module.exports = {
    id: "namefilter-remove-modal",
    noDeferred: true,
    ephemeral: true,
    type: `modal`,

    async execute(interaction, client) {
        const content = interaction.fields.getTextInputValue("namefilter-remove-input");
        const errorEmbed = []
        ...
        if (!database.invalidPseudos.includes(content)) {
            errorEmbed.push(new EmbedBuilder()
                .setTitle(`${client.config.emote.error} **ERREUR**`)
                .setColor("#920E15")
                .setDescription(`Le pseudo \`${content}\` n'est pas dans la liste des pseudos invalides.`)
            );
        }
        ...
        database.invalidPseudos = database.invalidPseudos.filter(pseudo => pseudo != content.trim());
        await Core.updateConfig("nameFilterConfig", database);

        const successEmbed = new EmbedBuilder()
            .setTitle(`${client.config.emote.success} **SUCCÈS**`)
            .setColor("#00BF63")
            .setDescription(`Le pseudo \`${content}\` a été retiré de la liste des pseudos invalides.`);
